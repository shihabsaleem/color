...
import { ColorSwatch, HarmonyType, generatePalette, hexToHsl, hexToRgb, colorName, hslToRgb, rgbToHex, regeneratePalette, rgbToHsl } from "@/lib/colorUtils";
...
// ΓöÇΓöÇΓöÇ Modal Wrappers ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
function HomeInner() {
  ...
  // On mount: load preset colors from URL param or generate a random palette
  useEffect(() => {
    const colorsParam = searchParams.get("colors");
    if (colorsParam) {
      const hexes = colorsParam.split(",").map((h) => `#${h}`);
      const loaded: ColorSwatch[] = hexes.slice(0, 5).map((hex, i) => {
        const rgb = hexToRgb(hex);
        if (!rgb) return null;
        const hsl = rgbToHsl(rgb.r, rgb.g, rgb.b);
        return { id: `swatch-${i}`, hex, hsl, rgb, locked: false, name: colorName(hex) };
  ...
  return (
    <main style={{ width: "100vw", height: "100vh", display: "flex", position: "relative", overflow: "hidden" }}>
          ...
          >
            ...
            <div className="swatch-controls" style={{ color: textColor }} onClick={(e) => e.stopPropagation()}>
              ...
              <button
                ...
                style={{ color: textColor }}
              >
                {isCopied ? <span style={{ fontSize: 18 }}>Γ£ô</span> : <IconCopy />}
              </button>
      ...
      <div style={{ position: "fixed", bottom: 40, left: 0, right: 0, display: "flex", justifyContent: "center", pointerEvents: "none", zIndex: 50 }}>
        ...
        >
          <div style={{ display: "flex", alignItems: "center", gap: 12, marginRight: 8, userSelect: "none" }}>
            ...
            <Link href="/" className="nav-link"><img src={SITE_INFO.logoUrl} alt="Logo" width={32} height={32} style={{ objectFit: "contain" }} />
              </Link></div>
          ...
          <div style={{ display: "flex", alignItems: "center", position: "relative" }}>
            ...
            <input
              ...
              onChange={(e) => {
                ...
                setSearchQuery(val);
                // Live search: if it's a valid hex or a named color, apply it
...
// ΓöÇΓöÇΓöÇ Modal Components ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
        ...
        <>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            <div style={{ padding: 12, background: "rgba(0,0,0,0.3)", borderRadius: "var(--radius-sm)", borderLeft: `3px solid ${res.aa_normal ? "#22c55e" : "#ef4444"}` }}>
              AA Normal (4.5:1) ΓÇö {res.aa_normal ? "Pass" : "Fail"}
            </div>
            <div style={{ padding: 12, background: "rgba(0,0,0,0.3)", borderRadius: "var(--radius-sm)", borderLeft: `3px solid ${res.aa_large ? "#22c55e" : "#ef4444"}` }}>
              AA Large (3:1) ΓÇö {res.aa_large ? "Pass" : "Fail"}
            </div>
          </div>
        </>
      )}
